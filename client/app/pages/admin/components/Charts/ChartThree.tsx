...
const ChartThree: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const activeResponse = await apiClient.get("/users/active-accounts");
        const inactiveResponse = await apiClient.get(
          "/users/inactive-accounts"
        );
        const unverifiedResponse = await apiClient.get(
          "/users/unverified-accounts"
        );
        const lockedResponse = await apiClient.get("/users/locked-accounts");
        const totalResponse = await apiClient.get("/users/total-accounts");
