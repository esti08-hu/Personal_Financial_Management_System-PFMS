...
import { RegisterUserDto } from 'src/modules/users/users.dto'
...
export class AuthController {
  ...
  async singIn(
    ...
    let authResponse: { accessToken: string; refreshToken: string; userId?: string }

    if (signInDto.isAdmin) {
      authResponse = await this.authService.signInAdmin(
        signInDto.email,
        signInDto.password,
      )
    } else {
      authResponse = await this.authService.signIn(
        signInDto.email,
        signInDto.password,
      )
    }

    const { accessToken: accessTokenValue, refreshToken: refreshTokenValue, userId } = authResponse
    ...
    res.cookie('access_token', accessTokenValue, {
      maxAge: rememberMe ? 1000 * 60 * 60 * 24 * 7 : accessTokenExpiration,
      httpOnly: true,
    })
    return { accessToken: accessTokenValue, refreshToken: refreshTokenValue, userId }
  }
